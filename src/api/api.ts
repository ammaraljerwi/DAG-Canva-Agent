...
const endpoints = {
  ...
  sendMessage: "/api/v1/messages/send_message",
  getMessages: "/api/v1/messages/get_history",
  queryAgent: "/api/v1/agents/call_agent"
};

export const sendAgentRequest = async (message: AgentRequestType) => {
  const url = new URL(endpoints.queryAgent, PYTHON_BACKEND)
  const options = { headers: { "Content-Type": "application/json" }, method: "POST", body: JSON.stringify(message) }
...
export const sendMessage = async (message: AgentMessageType) => {
  const url = new URL(endpoints.sendMessage, PYTHON_BACKEND)
  const options = { headers: { "Content-Type": "application/json" }, method: "POST", body: JSON.stringify(message) }
...
export const getMessages = async ({ user_id, session_id }): Promise<MessageType[]> => {
  const url = new URL(endpoints.getMessages, PYTHON_BACKEND);
  url.searchParams.append("user_id", user_id)
  url.searchParams.append("session_id", session_id)
  const result: MessageType[] = await sendRequest(url);
  return result
}
